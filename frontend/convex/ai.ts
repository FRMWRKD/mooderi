...
export const generateSmartBoard = action({
  ...
  handler: async (ctx, args): Promise<{
    ...
    for (const model of embeddingModels) {
      try {
        const response = await fetchWithRetry(
          `https://generativelanguage.googleapis.com/v1beta/models/${model}:embedContent?key=${googleApiKey}`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
    ...
    if (!embedding) {
      console.error("Smart board embedding failed:", lastError);
      throw new Error(
        `Smart Board unavailable: ${lastError}. To fix: Enable the "Generative Language API" in Google Cloud Console, or get a new API key from aistudio.google.com`
      );
    }
