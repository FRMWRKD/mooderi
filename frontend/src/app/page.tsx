...
export default function HomePage() {
    ...
    // 2. Images Data (Conditional)
    // If search query exists, use textSearch. Otherwise, use filter.
    ...
    const filterResults = useQuery(api.images.filter, !shouldSearch ? filterArgs : "skip");
    const searchResults = useQuery(api.images.textSearch, shouldSearch ? searchArgs : "skip");
    ...
    const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set()); // IDs are strings now
    ...
    const selectAll = () => {
        setSelectedIds(new Set(images.map((img: any) => img._id)));
    };
    ...
    const handleBatchSaveToBoard = async (boardId: Id<"boards">) => {
        ...
        for (const imageId of idsArray) {
            try {
                // imageId should be Id<"images">
                await addImageToBoard({ boardId, imageId: imageId as Id<"images"> });
                successCount++;
            } catch (e) {
                console.error(`Failed to save image ${imageId} to board ${boardId}`, e);
            }
        }
            ...
            <div className="space-y-6">
                            ...
                            <DropdownContent align="center" className="w-64">
                                ...
                                {boards.length > 0 ? (
                                    boards.map((board: any) => (
                                        <DropdownItem
                                            key={board._id}
                                            onClick={() => handleBatchSaveToBoard(board._id)}
                                        >
                ...
                {error && (
                    <div className="p-4 bg-red-500/10 border border-red-500/30 text-red-400">
                        ...
                        <button
                            onClick={() => setLimit(limit)} // Trigger re-render/retry
                ...
                {!isLoadingImages && images.length > 0 && viewMode !== "videos" && (
                    <div className={
                        viewMode === "grid"
                ...
                {/* Videos View - Group by videoId */}
                {!isLoadingImages && images.length > 0 && viewMode === "videos" && (() => {
                    // Group images by video
                    const videoGroups = images.reduce((acc: any, img: any) => {
                        if (img.videoId) {
                            if (!acc[img.videoId]) {
                                acc[img.videoId] = {
                                    videoId: img.videoId,
                                    sourceUrl: img.sourceVideoUrl || "",
                                    images: []
                                };
                            }
                            acc[img.videoId].images.push(img);
                        }
                        return acc;
                    }, {});

                    const videoEntries: any[] = Object.values(videoGroups).sort((a: any, b: any) => b.images.length - a.images.length);
                                    ...
                                    >
                                        ...
                                        <div className="relative aspect-video">
                                            ...
                                            <div className="absolute bottom-3 right-3 flex -space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                {group.images.slice(0, 4).map((img: any, i: number) => (
                                                    <img
                                                        key={img._id}
                                                        src={img.imageUrl}
                                                        alt=""
